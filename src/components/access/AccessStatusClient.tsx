...
export default function AccessStatusClient() {
  ...
  const [_accessRequest, _setAccessRequest] = useState<unknown>(null);

  const { syncAccessStatus } = useAccessSync();

  useEffect(() => {
    const fetchStatus = async () => {
      const { status: statusVal, hasUseCase, ok } = await syncAccessStatus();
      if (!ok) {
        router.push('/login');
        return;
      }
      ...
      if (statusVal === 'pending' && !hasUseCase) {
        router.replace('/access/request');
        return;
      }

      setStatus(statusVal as 'pending' | 'approved' | 'rejected');
    };
