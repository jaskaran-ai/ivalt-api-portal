...
export default function LoginClient() {
  ...
  const { syncAccessStatus, updateLocalState } = useAccessSync();

  useEffect(() => {
    // Sync the access status dynamically on client side
    syncAccessStatus().then(({ status, hasUseCase, ok }) => {
      if (ok && status) {
        if (status === 'approved') {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === 'authenticated') {
          ...
          updateLocalState(accessStatus, false);
  ...
  }, [fullNumber, router, updateLocalState]);
          ...
          const redirectPath =
            accessStatus === 'approved'
              ...
              : accessStatus === 'rejected'
                ? '/access/status'
                : '/access/request';
          updateLocalState(accessStatus, redirectPath === '/access/status');
