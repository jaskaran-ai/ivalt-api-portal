...
export const DEMO_ACCESS_REQUESTS = [
  ...
  },
  {
    ...
    useCase:
      'Developing a secure document signing platform with biometric verification for legal compliance.',
  ...
  },
  {
    ...
    useCase:
      'Building a biometric attendance system for corporate offices with real-time tracking.',
  ...
  },
  {
    ...
    useCase:
      'Integrating biometric authentication into a banking app for secure transaction approvals.',
  ...
  },
  {
    ...
    userId: DEMO_USERS[10].id,
    useCase:
      'Healthcare platform requiring biometric patient identity verification for telemedicine.',
...
export function getDemoAccessRequests() {
  return DEMO_ACCESS_REQUESTS.map((r) => {
    const userObj = DEMO_USERS.find((u) => u.id === r.userId);
    return {
      ...r,
      ...
      user: r.user
        ? {
            ...r.user,
            status: userObj ? userObj.status : 'pending',
          }
        : null,
    };
  });
...
export function deleteDemoUser(id: string) {
  ...
  // Delete user's keys
  _demoKeys = _demoKeys.filter((k) => k.userId !== id);
  // Delete user's requests
  const reqIndices = DEMO_ACCESS_REQUESTS.reduce((acc: number[], r, i) => {
    if (r.userId === id) acc.push(i);
    return acc;
  }, []);
  for (let i = reqIndices.length - 1; i >= 0; i--) {
    DEMO_ACCESS_REQUESTS.splice(reqIndices[i], 1);
  }
}

export function deleteDemoAccessRequest(id: string) {
  const index = DEMO_ACCESS_REQUESTS.findIndex((r) => r.id === id);
  if (index !== -1) {
    DEMO_ACCESS_REQUESTS.splice(index, 1);
  }
}
