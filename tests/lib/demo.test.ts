...
describe("DEMO_API_KEYS", () => {
  test("has 3 keys", () => {
    expect(DEMO_API_KEYS).toHaveLength(3);
  });

  test("keys belong to first user", () => {
    for (const key of DEMO_API_KEYS) {
      expect(key.userId).toBe(DEMO_USERS[0].id);
    }
  });
...
describe("DEMO_ACCESS_REQUESTS", () => {
  test("has 2 requests", () => {
    expect(DEMO_ACCESS_REQUESTS).toHaveLength(2);
  });
...
describe("getDemoAdminUsers", () => {
  ...
  test("first user has all keys", () => {
    const result = getDemoAdminUsers();
    expect(result[0].apiKeyCount).toBe(DEMO_API_KEYS.length);
  });
...
describe("getDemoAdminUsage", () => {
  ...
  test("summary has correct structure", () => {
    ...
    expect(summary).toHaveProperty("totalUsers", 3);
